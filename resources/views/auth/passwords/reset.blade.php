...
<head>
	...
	<title>{{trans('app.ResetPassword')}} | KuyRek</title>
</head>
			...
			<div class="content-push">
				...
				<div class="information-blocks">
						...
						<div class="col-sm-6 information-entry">
							<div class="login-box">
								<div class="article-container style-1">
									...
									<p>{{trans('app.Pleaseresetpassword')}}</p>
								</div>
                  ...
                  <input type="hidden" name="token" value="{{ $token }}">
									...
									<input class="simple-field" id="email" type="email" name="email" value="{{ $email or old('email') }}" required autofocus />
                  @if ($errors->has('email'))
                  ...
                  <label>{{trans('app.NewPassword')}}</label>
									<input class="simple-field" id="password" type="password" name="password" required />
                  @if ($errors->has('password'))
                      <span class="help-block alert alert-danger">
                          <strong>{{ $errors->first('password') }}</strong>
                      </span>
                  @endif
                  <label>{{trans('app.NewPasswordConfirmation')}}</label>
									<input class="simple-field" id="password-confirm" type="password" name="password_confirmation" required/>
                  @if ($errors->has('password_confirmation'))
                      <span class="help-block alert alert-danger">
                          <strong>{{ $errors->first('password_confirmation') }}</strong>
                      </span>
                  @endif
                  <div class="button style-4 col-md-12">{{trans('app.ResetPassword')}}<input type="submit" value="" /></div>
								</form>
							</div>
						</div>
						<div class="col-sm-6 information-entry">
