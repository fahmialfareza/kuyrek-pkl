...
<head>
  ...
  <title>{{trans('app.EditSportVenue')}} | Admin | KuyRek</title>
...
<body class="hold-transition skin-blue sidebar-mini">
          ...
          <div class="col-md-12">
            <div class="box box-primary">
              <div class="box-header with-border">
                <h3 class="box-title">{{trans('app.EditSportVenue')}}</h3>
              </div>
              <!-- /.box-header -->
              <form action="/admin/sportvenue/{{$sportvenue->id}}" method="post" enctype="multipart/form-data">
                {{csrf_field()}}
                <div class="box-body">
                  <div class="form-group">
                    <label for="inputName">{{trans('app.SportVenueName')}} <span>*</span></label>
                    <div>
                      <input type="text" class="form-control" id="name" name="name" value="{{$sportvenue->name}}" placeholder="{{trans('app.SportVenueName')}}" required>
                    </div>
                    @if ($errors->has('name'))
                  ...
                  <div class="form-group">
                    ...
                    <div>
                      <input type="text" class="form-control" id="tags" name="tags" value="{{$sportvenue->tags}}" placeholder="{{trans('app.Tags')}}">
                    </div>
                    @if ($errors->has('tags'))
                        <span class="help-block alert alert-danger">
                    ...
                    <select name="sportvenuecategory_id" class="form-control" required>
                      <?php foreach ($categories as $category): ?>
                        <option value="{{$category->id}}"
                          @if ($category->id == $sportvenue->sportvenuecategory_id)
                            selected
                          @endif
                    ...
                    <select name="sportvenuevendor_id" class="form-control" required>
                      <?php foreach ($vendors as $vendor): ?>
                        <option value="{{$vendor->id}}"
                          @if ($vendor->id == $sportvenue->sportvenuevendor_id)
                            selected
                          @endif
                        >{{$vendor->name}}</option>
                      <?php endforeach; ?>
                      ...
                      <select name="province_id" id="province_id" required>
                        <?php foreach ($provinces as $province): ?>
                          <option value="{{$province->id}}"
                              @if ($province->id == $sportvenue->district->regency->province->id)
                                selected
                              @endif
                      ...
                      <select name="regency_id" id="regency_id" required>
                        <?php foreach ($regencies as $regency): ?>
                          <option value="{{$regency->id}}"
                            @if ($regency->id == $sportvenue->district->regency->id)
                              selected
                            @endif
                          >{{$regency->name}}</option>
                        <?php endforeach; ?>
                      ...
                      <select name="district_id" id="district_id" required>
                        <?php foreach ($districts as $district): ?>
                          <option value="{{$district->id}}"
                            @if ($district->id == $sportvenue->district->id)
                              selected
                            @endif
                  ...
                  <div class="form-group">
                    ...
                    <select name="official" class="form-control" required>
                      <option value="0"
                        @if ($sportvenue->official == 0)
                          selected
                        @endif
                      >{{trans('app.NotOfficial')}}</option>
                      <option value="1"
                        @if ($sportvenue->official == 1)
                          selected
                        @endif
                      >{{trans('app.Official')}}</option>
                    </select>
                  ...
                  <input class="simple-field" type="text" id="lat" name="lat" value="{{$sportvenue->sportvenuedescription->lat}}" placeholder="Lat" hidden/>
                  <input class="simple-field" type="text" id="lng" name="lng" value="{{$sportvenue->sportvenuedescription->lat}}" placeholder="Lng" hidden/>
                  <div class="form-group">
                    <label for="inputName">{{trans('app.SportVenueFullAddress')}} <span>*</span></label>
                    <div>
                      <textarea type="text" class="form-control" id="address" name="address" placeholder="{{trans('app.SportVenueFullAddress')}}" required>{{$sportvenue->sportvenuedescription->address}}</textarea>
                    </div>
  ...
  <script type="text/javascript">
	  ...
	  function success(position) {
	    var lat = {{$sportvenue->sportvenuedescription->lat}};
	    var lng = {{$sportvenue->sportvenuedescription->lng}};
	    $('#lat').val(lat);
	    $('#lng').val(lng);
